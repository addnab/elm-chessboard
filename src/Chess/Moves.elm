module Chess.Moves exposing (applyMove, getBoardViewForNextMoves, isKingSafe)

import Actions exposing (Action(..))
import Chess.Board exposing (getPiece, movePiece, removePiece, setPiece, updateSquare)
import Chess.Pieces exposing (toPlayerPiece)
import Chess.Players exposing (opponent)
import Chess.Types exposing (Board, Move, MoveType(..), Piece(..), Player(..), PlayerInfo, PlayerPiece, Position, Square)
import Debug
import Dict
import Task
...
movesByDirection playerPiece board position maxHopCount direction =
    let
        ...
        nextPlayerPieceMaybe =
            getPiece newPosition board
        ...
        hopCount =
            Maybe.withDefault 8 maxHopCount
    in
...
isKingSafe isCastling playerPiece piecePosition currentKingPosition board nextMove =
    let
        ...
        newBoard =
            movePiece piecePosition (.position nextMove) board
    in
...
enableEnPassantPawns player position board =
    [ .file position + 1, .file position - 1 ]
        ...
        |> List.foldr
            (\( pos, playerPieceMaybe ) newBoard ->
                setPiece pos ( newBoard, playerPieceMaybe )
            )
            board
...
applyMove player fromPosition move playerInfo board =
    let
        ...
        ( newBoard, capturedPiece, promote ) =
            case .moveType move of
                ...
                Enpassant ->
                    let
                        capturePosition =
                            Position (.file toPosition) (.rank fromPosition)
        ...
        updatedBoard =
            disableEnPassantPawns player newBoard

        playerPieceMaybe =
            getPiece toPosition updatedBoard
